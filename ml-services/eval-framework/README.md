...
- `src/leaderboard.py` - ranks multiple models by a chosen metric; refuses
  to rank if models report incompatible or non-numeric metrics
...
- `tests/test_metrics.py` - 29 tests covering all of the above, including
...
- Wire the framework into a real teammate's model output for real (still
  deliberately deferred every round so far -- this round's demo builds its
  own Prophet fit rather than importing anyone else's code).
...
- Round 3 review caught a scipy import with no declared dependency file,
  which errors out all tests on a clean install - fixed with `requirements.txt`.
...
missing the metric being ranked on. This is also intentional: a ranking is a
definitive claim ("X is better than Y"), and that claim isn't trustworthy if
the models weren't even measured on the same thing. Silently skipping a
model or showing a partial ranking would be misleading.
...
- **`src/metrics.py`** - added `anomaly_metrics()`: recall, specificity,
  false positive rate, and balanced accuracy, specifically for anomaly
  detection where the normal class vastly outnumbers the anomaly class
  (plain accuracy is misleading there).
- **`src/leaderboard.py`** - `generate_leaderboard()` and `print_leaderboard()`.
  ...
  non-numeric value for it, or if fewer than 2 models are comparable.
...
A single metric on a single split can be misleading. This round adds two
more layers of honesty: the leaderboard refuses to compare apples to
oranges, and the significance test refuses to call a small improvement
"better" unless the data actually backs that up.
