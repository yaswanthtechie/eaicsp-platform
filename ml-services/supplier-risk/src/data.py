...
def load_headlines() -> Dict[str, List[str]]:
    ...
    Returns:
        Dict[str, List[str]]: A dictionary where keys are supplier names 
        and values are lists of headline strings.
    """
    headlines_raw = [
    ...
    for item in headlines_raw:
        ...
        if supplier not in grouped_headlines:
            grouped_headlines[supplier] = []
        grouped_headlines[supplier].append(headline)
