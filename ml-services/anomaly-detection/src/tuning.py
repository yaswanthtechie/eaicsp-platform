...
def load_data():
    ...
    test_datasets = {
        "temperature_spike": pd.read_csv(
            output_dir / "test_temperature_spike.csv"
        ),
        "temperature_drift": pd.read_csv(
            output_dir / "test_temperature_drift.csv"
        ),
        "stock_anomaly": pd.read_csv(
            output_dir / "test_stock_anomaly.csv"
        ),
        "combined_anomaly": pd.read_csv(
...
def run_tuning(model, train_features, test_datasets, name):
    print(f"\n{name}")
    return model.tune(
...
def main():
    ...
    summary = (
        ...
        .size()
        .reset_index(name="Top Configurations")
    )
