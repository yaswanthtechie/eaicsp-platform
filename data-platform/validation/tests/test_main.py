...
@patch("pandas.DataFrame.to_csv")
@patch("src.watermark.WatermarkManager")
def test_main_incremental_flow_new_output(mock_wm_class, mock_to_csv, mock_validator, mock_read, mock_args, mock_exists):
    """Tests incremental flow when output file doesn't exist yet (standard write branch)."""
    args = MagicMock()
    args.incremental = True
    args.skip_generate = True
    args.watermark_col = "transaction_id"
    ...
    # Assert to_csv was called directly and watermark updated
    mock_to_csv.assert_called_once()
    mock_wm_instance.set_watermark.assert_called_once_with(20)
...
def test_main_list_profiles_found(mock_list, mock_args, mock_exists):
...
def test_main_list_profiles_not_found(mock_list, mock_args, mock_exists):
