...
def test_validate_row_crashing_evaluate(caplog):
    """Tests that a crashing rule evaluation does not halt execution but correctly fails the row."""
    r_e = ConfigRule(name="crash_e", type="custom", function="crashing_custom_rule", severity="ERROR")
    val = DataValidator([r_e])
    res = val.validate_row({"A": 1})

    assert res.passed is False
    assert "crash_e" in res.errors
...
def test_validate_records_field_remediations():
    ...
    rule = ConfigRule(**{
        ...
        "target_case": "upper",
        "severity": "INFO",
        "description": "Converted strings to uppercase"
    })
    ...
    assert rem["reason"] == "Converted strings to uppercase"
...
def test_validate_ignores_column_addition_remediations():
    """Tests that validate() ignores column additions so they don't artificially inflate modification counts."""
    df = pd.DataFrame({"A": [1, 2]})
    ...
    # Because the schema changed (column added), changed_count is forced to 0
...
def test_resolve_env_path_no_env():
    """Tests that providing no environment safely falls back to 'dev'."""
    original = "configs/sales_rules.yaml"
    expected = Path("configs/dev/sales_rules.yaml")

    assert resolve_env_path(original) == expected
    assert resolve_env_path(original, env=None) == expected


def test_resolve_env_path_ignored_envs():
    """Tests that internal or local fallback environments securely route to 'dev'."""
    original = "configs/sales_rules.yaml"
    ignored_envs = ["", "local", "default", "none", "LOCAL", "None"]
    expected = Path("configs/dev/sales_rules.yaml")

    for env in ignored_envs:
        assert resolve_env_path(original, env=env) == expected
...
def test_resolve_env_path_invalid_env():
    """Tests that arbitrary directory traversal or unknown envs raise a ValueError."""
    with pytest.raises(ValueError, match="Invalid environment"):
        resolve_env_path("configs/rules.yaml", env="../../x")