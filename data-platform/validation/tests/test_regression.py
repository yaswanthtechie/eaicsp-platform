...
def test_golden_regression():
    ...
    assert report.total_rows_affected == 380
    ...
    assert error_counts.get('unparseable_dates') == 338
    ...
    assert error_counts.get('composite_pk_unique') == 35
    assert warning_counts.get('quantity_positive') == 2
    assert warning_counts.get('date_in_range') == 22
